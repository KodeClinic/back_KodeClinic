...
router.patch("/:idSpecialist", async (req, res, next) => {
  try {
...
router.get("/:idSpecialist/patients", async (req, res, next) => {
  try {
    const { idSpecialist } = req.params;

    const listOfPatients = await specialistsUseCases.patientList(idSpecialist);
